...
class FlowoffParticle {
  ...
  display() {
    ...
    if(this.depth > 0.7) { // Apenas estrelas próximas têm brilho
      // Brilho central
    ...
    if(this.depth < 0.5) { // Estrelas distantes ficam desfocadas
...
function drawConnections() {
  ...
  for(let i = 0; i < particles.length; i++) {
    for(let j = i + 1; j < particles.length; j++) {
      ...
      if(distance < 80) { // Distância menor para estrelas
